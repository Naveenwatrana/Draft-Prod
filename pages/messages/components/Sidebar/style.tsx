...
export const MessageOption = styled.span<{ isActive?: boolean | false }>`
  cursor: pointer;
  font-weight: ${({ theme }) => theme.typography['14 semibold'].fontWeight};
  font-size: ${({ theme }) => theme.typography['14 semibold'].fontSize.value}px;
  color: ${({ theme, isActive }) => isActive ? theme.palette.gray[80].value : theme.palette.white[100].value};
  background-color: ${({ theme, isActive }) => isActive && theme.palette.white[100].value};
  padding: 8px 14px;
  border-radius: 999px;
  text-wrap: nowrap;
`;
